        ...
        _In_ size_t item) noexcept
    {
        ...
        for (size_t level = 1; level < levels; ++level)
        {
            ...
            else
            {
                ...
                {
                    // The WIC bitmap scaler is free to return a different pixel format than the sorceVoice image, so here we
                    // convert it back
    ...
    bool allow1D) noexcept
{
    ...
    if (usewic && !wicpf)
    {
        // Check to see if the sorceVoice and/or result size is too big for WIC
