...
//静的メンバ変数の実体
const float MultipleTexture::clearColor[4] = { 0.25f,0.5f,0.1f,0.0f };//緑っぽい色
...
void MultipleTexture::CreateGraphicsPipelineState()
{
	...
	(
		L"Resources/shaders/MultipleTextureVS.hlsl",	// シェーダファイル名
		nullptr,
	...
	result = D3DCompileFromFile
	(
		L"Resources/shaders/MultipleTexturePS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
void MultipleTexture::Initialize()
{
	HRESULT result;
...
void MultipleTexture::PreDrawScene(ID3D12GraphicsCommandList *cmdList)
{
	for (int i = 0; i < 2; i++)
	{
...
void MultipleTexture::Draw(ID3D12GraphicsCommandList *cmdList)
{
	// 定数バッファにデータ転送
...
void MultipleTexture::PostDrawScene(ID3D12GraphicsCommandList *cmdList)
{
	for (int i = 0; i < 2; i++)
	{
