...
//静的メンバ変数の実体
const float MultipleRender::clearColor[4] = { 0.25f,0.5f,0.1f,0.0f };//緑っぽい色

MultipleRender::MultipleRender()
	: Sprite
	(
		100,//テクスチャ番号
		{ 0, 0},//座標
...
void MultipleRender::CreateGraphicsPipelineState()
{
...
void MultipleRender::Initialize()
{
	HRESULT result;
...
void MultipleRender::PreDrawScene(ID3D12GraphicsCommandList *cmdList)
{
	for (int i = 0; i < 2; i++)
	{
...
void MultipleRender::Draw(ID3D12GraphicsCommandList *cmdList)
{
...
void MultipleRender::PostDrawScene(ID3D12GraphicsCommandList *cmdList)
{
	for (int i = 0; i < 2; i++)
	{
